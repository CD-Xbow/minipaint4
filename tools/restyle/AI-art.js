class Transferer {
  constructor() {
    ...
    this.contentImg = this.contentImgWrapper.querySelector("img");
    ...
    this.uploadStyleButton = document.getElementById("uploadStyle");
  ...
  async init() {
    ...
    this.generateArtButton.addEventListener("click", this.applyStyle.bind(this));
    ...
    this.uploadStyleButton.addEventListener("click", () => this.styleInput.click());
    this.styleInput.addEventListener("change", this.uploadStyle.bind(this));
  ...
  async uploadStyle(e) {
    if (e.target.files[0]) {
      this.styleImg.src = window.URL.createObjectURL(e.target.files[0]);
      await new Promise((res) => {
